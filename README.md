...
| `route_incoming_document` | Files every attachment on a received email onto one record by `file_url` (one stored copy, no base64), links the email to it as the Desk's Relink does, and leaves an audit comment. | Route an email that is already linked, or leave one half-routed — a refusal takes back every file. |
...
| `create_training_type` | A curriculum for a course that has **not run yet** — name, description, regimes, delivery method, duration, retention — so a Training Session can be booked against it. Regimes left out are inferred from the name, as `record_training` infers them, and the reply says so. | Update a curriculum that already exists (any casing) — that is `update_training_type`. Nor accept an unknown regime or delivery method, or store a retention shorter than its regimes require. |
| `deactivate_training_type` | Retires a curriculum (`active = 0`) with a stated reason on its timeline. It then drops out of the compliance matrix and the curriculum listing, and no new session can be booked against it. Open sessions are named. | Delete it, or touch any training record or session that names it. `record_training` still accepts it, so paperwork for a class that already ran can still be filed. |
